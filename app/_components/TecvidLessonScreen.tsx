...
export default function TecvidLessonScreen({lesson}:{lesson:TecvidLesson}){
 ...
 return <main className="min-h-screen bg-[#f7f3ec] text-[#183f3a]"><audio ref={audio} preload="metadata"/><AcademyHeader title="10 Derslik Tecvid Programı" activeLesson={`Ders ${String(lesson.day).padStart(2, "0")} · ${lesson.shortTitle}`}/>
  <div className="mx-auto grid max-w-[1450px] gap-7 px-4 py-7 lg:grid-cols-[300px_minmax(0,1fr)] lg:px-7"><aside className="lg:sticky lg:top-24 lg:h-[calc(100vh-7rem)]"><Paper elevation={0} className="!rounded-3xl !p-4 ring-1 ring-blue-950/10"><div className="mb-4 px-2"><p className="text-xs font-black tracking-[.15em] text-[#b65f38]">ÖĞRENME YOLUN</p><b className="mt-1 block text-lg">10 derslik Tecvid</b><div className="mt-3 flex items-center gap-3"><LinearProgress variant="determinate" value={progress} className="flex-1" sx={{height:8,borderRadius:5,bgcolor:"#e7e2da","& .MuiLinearProgress-bar":{bgcolor:"#244a70"}}}/><span className="text-xs font-black">%{progress}</span></div></div><nav aria-label="Tecvid dersleri" className="space-y-1">{tecvidLessons.map(item=><Link key={item.slug} href={`/tecvid/${item.slug}`} className={`flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-bold transition ${item.day===lesson.day?"bg-blue-50 text-[#244a70]":"text-blue-950/55 hover:bg-stone-50"}`}><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl text-xs font-black text-white" style={{backgroundColor:completed.includes(item.day)?"#17756a":item.color}}>{completed.includes(item.day)?"✓":String(item.day).padStart(2,"0")}</span><span className="line-clamp-1">{item.shortTitle}</span></Link>)}</nav></Paper></aside>
   <section className="min-w-0 space-y-6"><Paper elevation={0} className="!rounded-[2rem] !bg-[#244a70] !p-7 !text-white sm:!p-10"><div className="flex flex-wrap items-center justify-between gap-3"><Chip label={`DERS ${String(lesson.day).padStart(2,"0")} · ${lesson.category.toUpperCase()}`} sx={{bgcolor:"#f6cf88",color:"#183f3a",fontWeight:900}}/><Link href="/tecvid" className="inline-flex items-center gap-2 text-sm font-bold text-blue-100"><ArrowBackRounded fontSize="small"/> Ders kartları</Link></div><h1 className="mt-5 font-serif text-4xl font-bold sm:text-6xl">{lesson.title}</h1><p className="mt-4 max-w-3xl text-lg leading-8 text-blue-100">{lesson.summary}</p><div className="mt-7 rounded-3xl bg-white/10 p-6 ring-1 ring-white/15"><p className="text-xs font-black tracking-[.16em] text-[#f6cf88]">BUGÜNÜN KURALI</p><p className="mt-3 text-lg font-semibold leading-8">{lesson.rule}</p></div></Paper>
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_310px]"><Paper elevation={0} className="!rounded-3xl !p-6 ring-1 ring-blue-950/10"><p className="text-xs font-black tracking-[.16em] text-[#b65f38]">3 ADIMDA ÇALIŞ</p><div className="mt-4 grid gap-3 sm:grid-cols-3">{["Önce yalnız dinle.","Renkli bölümü izleyerek tekrar et.","Ekrana bakmadan üç kez oku."].map((text,index)=><div key={text} className="rounded-2xl bg-blue-50 p-4 text-sm font-bold leading-6"><span className="mr-2 text-[#b65f38]">{index+1}.</span>{text}</div>)}</div></Paper><Paper elevation={0} className="!rounded-3xl !p-6 ring-1 ring-blue-950/10"><p className="text-[11px] font-black uppercase tracking-wider text-blue-950/45">Kaynakça</p><div className="mt-3 space-y-3"><a href="https://kuran.diyanet.gov.tr/kuran-sozlugu/detay/51-tecvid" target="_blank" rel="noreferrer" className="block text-xs font-bold leading-5 text-[#244a70] underline decoration-blue-200 underline-offset-4">Diyanet Kur’an Sözlüğü<br/><span className="font-medium">Tecvid maddesi</span></a><a href="https://kurul.diyanet.gov.tr/tr/fetvalar/kuranin-kiraati/e4c6b440-d8da-48b0-a0dc-aeb000d9620d/tecvid-kurallari/01933666-3c40-7a54-2426-1fb669131440" target="_blank" rel="noreferrer" className="block text-xs font-bold leading-5 text-[#244a70] underline decoration-blue-200 underline-offset-4">Din İşleri Yüksek Kurulu<br/><span className="font-medium">Tecvid kuralları ve tertîl</span></a></div></Paper></div>
